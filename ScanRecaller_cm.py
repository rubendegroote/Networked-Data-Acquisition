 ...
 used to bin the data.')
    ...
    def request_processed_data_reply(self,track,params):
        ...
        if done:
            ...
            # self.centers = doppler_shift(self.centers,29945.4747,64.9277897)
    ...
    def plot(self):
        if 'wavenumber' in self.xkey[1]:
            ...
            self.plotWidget.setLabel(text='offset: {} cm-1 (unit=cm-1)'.format(offset),
                                     axis='bottom')
        else:
            x = self.centers
...
class FitWidget(QtGui.QWidget):
    ...
    def init_UI(self): 
        ...
        self.centroid = QtGui.QDoubleSpinBox(value=401340000*3,
                                   singleStep = 1,
                                   decimals = 0, suffix='cm^-1',
                                   maximum=4013400000000)
