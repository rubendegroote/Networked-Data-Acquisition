...
LOG_PATH = 'C:\\Tuning\\logbook'
INI_PATH = 'C:\\Tuning\\scan_init.ini'

class Tuner(Dispatcher):
    def __init__(self, PORT=5007, name='Tuner'):
        super(Tuner, self).__init__(PORT, name)
        ...
        self.tune_parser = configparser.ConfigParser()
        try:
            self.tune_parser.read(INI_PATH)
            # read in stuff from ini file here
        except:
...
def makeTuner(PORT=5007):
    return Tuner(PORT=PORT)
