...
from acquire import acquire,acquireWavemeter

SAVE_INTERVAL = 2

class LaserArtist(asyncore.dispatcher):
    """
    Parameters
    ----------
    name: str
    ...
    def __init__(self, name='', settings={}, PORT=5005, save_data=True):
        super(LaserArtist, self).__init__()
        self.port = PORT
    ...
    def StartDAQ(self):
        ...
        self.DAQProcess = mp.Process(target=acquireWavemeter,
                                     args=(self.settings,
    ...
    def handle_close(self):
        ...
        super(LaserArtist, self).handle_close()

def makeArtist(name='test1', PORT=5005, save_data=True):
    return LaserArtist(name=name, PORT=PORT, save_data=save_data)
