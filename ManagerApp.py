...
class ManagerApp(QtGui.QMainWindow):
    ...
    def addConnection(self,data):
        self.manager = Manager(artists=[(data[0],data[1])])
        if not self.looping:
            self.looping = True
    ...
    def update(self):
        try:
            self.scanner.updateProgress(self.manager.scanProgress)
            if self.scanner.state == 'STOP' and not self.manager.scanning:
